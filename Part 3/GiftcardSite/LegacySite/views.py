...
def buy_card_view(request, prod_num=0):
    if request.method == 'GET':
        ...
        card_data = extras.generate_card_data(prod, request.user)
        card_file_name = f"card_{request.user}_{num_cards+1}.png"
        card_file_path = default_storage.save(card_file_name, ContentFile(card_data))
        card.save()
